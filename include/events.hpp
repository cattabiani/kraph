...
class NewEdgeEvent : public Event {
  public:
    NewEdgeEvent(const string &from, const string &to, const bool is_triggered)
    ...
    friend std::ostream &operator<<(std::ostream &os, const NewEdgeEvent &obj) {
        const Event &base = obj;
        return os << base;
    }
};
...
class FlipEdgePlugEvent : public Event {
  public:
    FlipEdgePlugEvent(const string &id, bool is_from, const bool is_triggered)
