...
class Edge {
  public:
    ...
    Edge(const K::KJsonParser::dict_type &d)
        : id_(get<string>(*d.at("id_"))), label_(get<string>(*d.at("label_"))),
          info_(get<string>(*d.at("info_"))),
          from_(get<string>(*d.at("from_"))), to_(get<string>(*d.at("to_"))),
    ...
    friend std::ostream &operator<<(std::ostream &os, const Edge &obj) {
        ...
        os << '{' << ps{"id_", obj.id_} << sep << ps{"label_", obj.label_}
           << sep << ps{"info_", obj.info_} << sep << ps{"from_", obj.from_}
           << sep << ps{"to_", obj.to_} << sep
